public class UseStringBuffer {
    public static void main(String[] args) {
        ...
        String[] words = {"Too" , "many", "cooks", "spoi", "the" , "broth"};
        sentence.append(words[0]);
        for(int i = 1 ; i<words.length ; i++) 
            sentence.append(' ').append(words[i]);

        System.out.println("\nString in StringBuffer object is:\n" +sentence.toString());
        System.out.println("StringBuffer object capacity is now "+ sentence.capacity()+" and string length is "+sentence.length());
